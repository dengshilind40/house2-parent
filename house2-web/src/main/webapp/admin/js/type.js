        ...
        columns:[[
            ...
            {field:'dd',title:'操作',width:100,
                formatter: function(value,row,index){
                    return "<a href='javascript:deleteType("+row.id+")'>删除</a>  <a href=''>修改</a>"
                }
            },
...
function Updateload() {
    ...
    if(rows.length==1){
        ...
        var bh=rows[0].id;
        $.post("selectByOneType.do",{"id":bh},function (data) {
...
function deleteType(id) {
    //确认提示框
    $.messager.confirm('删除类型', '确定要删除吗?', function(r){
        if (r){
            //异步请求   删除
            $.post("deleteType.do",{"id": id},function (data) {
                if (data.result == 1) {
                    $("#dg").datagrid("reload")//刷新
...
function DeleteMore() {
    ...
    if(rows.length==0){
        ...
        $.messager.confirm("删除类型","确定删除吗？",function (r) {
            if(r){
                var ids='';
                ...
                $.post("delMoreType.do",{"ids":ids},function (data) {
                    if(data.result>=1){
                        ...
                        $.messager.alert('提示信息', '类型删除失败', 'error');
                    }
                },"json");
            }
